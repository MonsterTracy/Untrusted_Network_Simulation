...
def test_villager_and_werewolf_hard_knowledge():
    env = _env()
    assert env.observer_private_knowledge(5) == (
        [],
        ["player5"],
    )
    assert env.observer_private_knowledge(1) == (
        ["player1", "player2"],
...
def test_seer_checks_enter_the_correct_hard_set(
    ...
    known_wolves, known_non_wolves = (
        env.observer_private_knowledge(3)
    )
...
def test_witch_keeps_legal_knife_target_and_sees_no_later_target_after_heal():
    ...
    env.step(("kill", 5))
    env.step(("kill", 5))
    assert env.observer_private_knowledge(4) == (
        [],
        ["player4", "player5"],
    )
    env.step(("check", 1))
    ...
    env.step(("kill", 6))
    env.step(("kill", 6))
    known_wolves, known_non_wolves = env.observer_private_knowledge(4)
    assert known_wolves == []
...
def test_witch_poison_target_does_not_enter_hard_knowledge():
    ...
    env.step(("kill", 5))
    env.step(("check", 1))
    before = env.observer_private_knowledge(4)
...
def test_witch_knife_target_is_isolated_from_other_observers():
    ...
    assert env.observer_private_knowledge(3) == (
        [],
        ["player3"],
    )
    assert env.observer_private_knowledge(5) == (
...
def test_death_log_does_not_add_identity_knowledge():
    ...
    assert env.observer_private_knowledge(6) == (
        [],
        ["player6"],
    )
